...
describe('Sublists', () => {
	...
	test('remove all lines on an already empty sublist', () => {
		...
		test('getText() on field', () => {
			...
			record.getSublistValue.mockImplementation(() => {
				throw new Error();
			});
	...
	describe('toggling dynamic mode/standard mode api', () => {
		test('set standard mode on dynamic record', () => {
			const fakeRec = record.create({ type: 'fake', isDynamic: true });
	...
	describe('Sublist iteration', () => {
		test('iterate over sublist lines', () => {
