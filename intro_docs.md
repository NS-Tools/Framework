...
NS-Tools/Framework is a fork of
exploreconsulting/netsuite-fasttrack-toolkit-ss2. See the project
[readme][readme] for more details about the project.

<!-- prettier-ignore -->
[readme]: https://github.com/NS-Tools/Framework/blob/master/readme.md

# Original Readme _Lightly Edited_

This is the original README from the orignal NetSuite Fasttrack Toolkit.
It has been lightly edited and kept for those who are familiar with the
existing documents, but may have some out-of-date information. Please
use the [NS Tool Wiki][wiki] and the [NS Tools
Documentation][doc-site] for the most current information.

<!-- prettier-ignore -->
[wiki]: https://github.com/NS-Tools/Framework/wiki

<!-- prettier-ignore -->
[doc-site]: https://nstools.dev/
...
See API [docs here](https://nstools.dev/)

![NFT Intro Image](media/images/NFT.png)

## Getting Started (Typescript)

Please use the [getting started section of the readme][readme] for
installation.

[readme]: https://nstools.dev/modules/governance.html
...
**see also [`example.ts`][example]**

<!-- prettier-ignore -->
[example]: https://github.com/NS-Tools/Framework/blob/master/example.ts
...
`nsSearchResult2obj` turns a netsuite `search.Result` into a "plain-old
JavaScript Object" (POJO), especially useful when chaining operations or
returning
...
let firstResultAsObj = Seq(LazySearch.load("123"))
  .map(nsSearchResult2obj<{ foo; bar }>())
  .first();
```

Also see [search](https://nstools.dev/modules/search.html) in the API
documentation, especially the `LazySearch` class.
...
const customers = Seq(
  LazyQuery.from(
    {
      query: `SELECT ID AS FOO FROM Customer WHERE subsidiary = ?`,
      params: [1],
    },
    50,
  ),
...
Also see [governance](https://nstools.dev/modules/governance.html) API
docs.

## Logging

NFT provides an advanced logging mechanism based on [Aurelia's
logger](https://github.com/aurelia/logging).

It means you can have multiple loggers and control the logging verbosity
of each. In other words, it's a lightweight but much richer logging
facility than the NetSuite provided logger.
...
```typescript
export const afterSubmit = autolog(function afterSubmit(
  ctx: EntryPoints.UserEvent.afterSubmitContext,
) {});
```

Other configuration options include automatic logging of execution time,
governance usage, and other goodies.
...
Please do. Reach out to @quackenbushdev or @accountingnerd for more
...
Configure tsconfig to include `paths` for NetSuite type modules:
...
- Typescript definitions (`N/\*.d.ts` files) are defined via the
...
tests must be deployed in order to execute on a NetSmodulesuite account.
...
\_NOTE: Deprecated. [TODO]: Update the documentation.
