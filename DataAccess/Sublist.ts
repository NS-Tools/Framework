...
export class Sublist<out T extends SublistLine> implements Iterator<T> {
	...
	addLine(ignoreRecalc = true, insertAt: number = this.length): T {
		...
		if (this._useDynamicModeAPI && this.nsrecord.isDynamic) this.nsrecord.selectNewLine({ sublistId: this.sublistId });
		else {
...
export import SublistFieldType = importSublistFieldType;